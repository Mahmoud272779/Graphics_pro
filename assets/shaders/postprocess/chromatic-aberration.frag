...
void main(){
   ...
   float shiftleft=tex_coord-vec2(STRENGTH,0);
   float shiftright=tex_coord+vec2(STRENGTH,0);
    ...
    frag_color = vec4(texture(tex,shiftleft).r, texture(tex,tex_coord).g,texture(tex,shiftright).b , 1.0);
}